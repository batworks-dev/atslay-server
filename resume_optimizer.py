...
def optimize_resume_to_latex(resume_text: str, is_latex_source: bool = False) -> dict:
    ...
    prompt = (
        ...
        "Optimize every bullet for ATS. The output must fill ONE complete page — not half, not more.\n\n"

        "=== STEP 1: COUNT YOUR SECTIONS ===\n"
        "Before writing a single line, count:\n"
        "  E = number of work experience entries in the candidate's resume\n"
        "  P = number of project entries in the candidate's resume\n\n"

        "=== STEP 2: PICK YOUR PROFILE FROM THIS TABLE ===\n"
        "Use the E and P counts to select a profile. Follow it exactly.\n\n"
        "  PROFILE A  (E<=1, P<=2):\n"
        "    → 4 bullets per experience, 4 bullets per project\n"
        "    → Each bullet: 160-200 visible characters (will wrap to 2-3 lines)\n"
        "    → Skills: 6 lines. Extracurricular: 4 lines.\n\n"
        "  PROFILE B  (E=2, P<=2):\n"
        "    → 4 bullets per experience, 3 bullets per project\n"
        "    → Each bullet: 130-160 visible characters (wraps to ~2 lines)\n"
        "    → Skills: 6 lines. Extracurricular: 3-4 lines.\n\n"
        "  PROFILE C  (E=2, P=3)  [MOST COMMON]:\n"
        "    → 3 bullets per experience, 3 bullets per project\n"
        "    → Each bullet: 120-150 visible characters (wraps to ~1.5-2 lines)\n"
        "    → Skills: 6 lines. Extracurricular: 3 lines.\n\n"
        "  PROFILE D  (E=2, P=4) or (E=3, P=2):\n"
        "    → 3 bullets per experience, 3 bullets per project\n"
        "    → Each bullet: 100-125 visible characters (wraps to ~1-1.5 lines)\n"
        "    → Skills: 6 lines. Extracurricular: 3 lines.\n\n"
        "  PROFILE E  (E=3, P=3) or (E=2, P>=5):\n"
        "    → 3 bullets per experience, 2 bullets per project\n"
        "    → Each bullet: 90-110 visible characters (exactly 1 line)\n"
        "    → Skills: 5 lines. Extracurricular: 2-3 lines.\n\n"
        "  PROFILE F  (E>=3, P>=4):\n"
        "    → 2 bullets per experience, 2 bullets per project\n"
        "    → Each bullet: 80-100 visible characters (exactly 1 line)\n"
        "    → Skills: 5 lines. Extracurricular: 2 lines.\n\n"
        "VISIBLE CHARACTER COUNT = count only readable text, ignore LaTeX commands like \\textbf{}.\n"
        "Example: \"Deployed the system on \\textbf{AWS EC2}, achieving \\textbf{99.9\\%} uptime\""
        " has 60 visible chars.\n\n"

        "=== STEP 3: WRITE THE BULLETS ===\n"
        "Every bullet MUST follow: Action Verb + What was done + Quantified Metric.\n"
        "Wrap ALL numbers in \\textbf{}: \\textbf{500+} users, \\textbf{$\\sim$40\\%} reduction.\n"
        "If the candidate has no metric for a bullet, INVENT a plausible one:\n"
        "  latency → sub-\\textbf{200ms}, users → \\textbf{1,000+} daily active users,\n"
        "  performance → \\textbf{35\\%} faster, errors → \\textbf{40\\%} fewer failures.\n"
        "Strong verbs: Architected, Engineered, Spearheaded, Deployed, Automated, Orchestrated.\n"
        "No weak phrases: 'responsible for', 'helped', 'worked on', 'assisted with'.\n\n"
        ...
        "=== STEP 5: ONE-PAGE GUARANTEE ===\n"
        "You selected a profile in Step 2. Following that profile EXACTLY is what keeps the resume\n"
        "on one page. Do not add extra bullets. Do not shorten bullets below the minimum.\n"
